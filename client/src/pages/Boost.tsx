...
type Step = 'input' | 'analyzing' | 'results' | 'creative' | 'targeting' | 'budget' | 'preview' | 'launch'
            ...
            {['results', 'creative', 'targeting', 'budget', 'preview', 'launch'].map((s, i) => (
              <div key={s} className="flex items-center">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium transition-colors ${
                  step === s ? 'bg-primary text-background-dark' :
        ...
        {step === 'results' && analysis && (
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="p-6 rounded-2xl bg-surface-light dark:bg-surface-dark border border-border-light dark:border-border-dark">
                <div className="flex items-center gap-2 mb-4">
                  <span className={`material-symbols-outlined ${analysis.pixelDetected ? 'text-green-500' : 'text-yellow-500'}`}>
                    {analysis.pixelDetected ? 'check_circle' : 'warning'}
                  </span>
                  <span className="font-semibold text-text-primary-light dark:text-text-primary-dark">Meta Pixel</span>
                </div>
                <div className={`text-lg font-semibold ${analysis.pixelDetected ? 'text-green-500' : 'text-yellow-500'}`}>
                  {analysis.pixelDetected ? 'Detected' : 'Not Found'}
                </div>
                <p className="text-sm text-text-secondary-light dark:text-text-secondary-dark">
                  {analysis.pixelDetected ? 'Ready for tracking' : 'Consider adding pixel'}
                </p>
              </div>
            </div>
              ...
              <div className="p-6 rounded-2xl bg-surface-light dark:bg-surface-dark border border-border-light dark:border-border-dark">
                ...
                <div className="grid grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-2">
                  {analysis.images.slice(0, 8).map((img, i) => (
                    <div key={i} className="aspect-square rounded-lg overflow-hidden bg-background-dark/10">
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    {analysis.adCopy.map((ad, i) => (
                      <button
                        ...
                        className={`w-full p-4 rounded-xl text-left transition-colors ${
                          selectedHeadline === i && !customHeadline
                  ...
                  <div className="space-y-2">
                    {analysis.adCopy.map((ad, i) => (
                      <button
                        ...
                        className={`w-full p-4 rounded-xl text-left transition-colors ${
                          selectedPrimaryText === i && !customPrimaryText
                            ? 'bg-primary/20 border-2 border-primary'
                            : 'bg-surface-light dark:bg-surface-dark border-2 border-transparent hover:border-primary/30'
                        }`}
                      >
                      ...
                      {analysis.images.slice(0, 8).map((img, i) => (
                        <button
                          ...
                          className={`aspect-square rounded-lg overflow-hidden transition-all ${
                            selectedImage === i ? 'ring-4 ring-primary scale-105' : 'hover:ring-2 ring-primary/30'
                          }`}
                        >
                          <img src={img} alt="" className="w-full h-full object-cover" onError={(e) => (e.currentTarget.style.display = 'none')} />
                        </button>
              ...
              <div className="lg:sticky lg:top-8">
                <div className="p-6 rounded-2xl bg-surface-light dark:bg-surface-dark border border-border-light dark:border-border-dark">
                  <div className="text-sm font-medium text-text-secondary-light dark:text-text-secondary-dark mb-4">Live Preview</div>
                  ...
                  <div className="bg-white dark:bg-gray-900 rounded-xl overflow-hidden shadow-lg">
                    ...
                    {analysis.images[selectedImage] && (
                      <div className="aspect-square bg-gray-100 dark:bg-gray-800">
                        <img src={analysis.images[selectedImage]} alt="" className="w-full h-full object-cover" />
                      </div>
                    )}
            ...
            <div className="max-w-md mx-auto space-y-6">
              <div className="p-6 rounded-2xl bg-surface-light dark:bg-surface-dark border border-border-light dark:border-border-dark">
                ...
                <ul className="space-y-3">
                  ...
                  <li className="flex items-center gap-3">
                    <span className="material-symbols-outlined text-primary">check_circle</span>
                    <span className="text-text-primary-light dark:text-text-primary-dark">Retargeting website visitors</span>
                  </li>
                  <li className="flex items-center gap-3">
                    <span className="material-symbols-outlined text-primary">check_circle</span>
                    <span className="text-text-primary-light dark:text-text-primary-dark">Pixel health insights</span>
                  </li>
                  <li className="flex items-center gap-3">
                    <span className="material-symbols-outlined text-primary">check_circle</span>
