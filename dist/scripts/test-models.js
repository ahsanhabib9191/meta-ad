...
const performance_snapshot_1 = require("../lib/db/models/performance-snapshot");
const optimization_log_1 = require("../lib/db/models/optimization-log");
...
async function run() {
    ...
    if (campaign.budget !== 100)
        throw new Error('Campaign budget incorrect');
    if (campaign.objective !== 'OUTCOME_SALES')
        throw new Error('Campaign objective incorrect');
    // Test Campaign update
    campaign.status = 'PAUSED';
    campaign.budget = 150;
    await campaign.save();
    const updatedCampaign = await campaign_1.CampaignModel.findOne({ campaignId: campaign.campaignId });
    if (!updatedCampaign)
        throw new Error('Campaign not found after update');
    if (updatedCampaign.status !== 'PAUSED')
        throw new Error('Campaign status not updated');
    if (updatedCampaign.budget !== 150)
        throw new Error('Campaign budget not updated');
    // Test Campaign unique constraint
    try {
        await campaign_1.CampaignModel.create({
    ...
    if (!adSet.adSetId)
        throw new Error('AdSet creation failed');
    if (!adSet.targeting)
        throw new Error('AdSet targeting not saved');
    if (adSet.targeting.audienceSize !== 100000)
        throw new Error('Targeting audienceSize incorrect');
    if (adSet.targeting.ageMin !== 18)
        throw new Error('Targeting ageMin incorrect');
    ...
    await adSet.save();
    const updatedAdSet = await ad_set_1.AdSetModel.findOne({ adSetId: adSet.adSetId });
    if (updatedAdSet?.learningPhaseStatus !== 'ACTIVE')
        throw new Error('Learning phase status not updated');
    ...
    if (!ad.creative)
        throw new Error('Ad creative not saved');
    if (ad.creative.headline !== 'Amazing Product')
        throw new Error('Creative headline incorrect');
    if (!ad.issues || ad.issues.length !== 1)
        throw new Error('Ad issues not saved');
    if (ad.issues[0].level !== 'WARNING')
        throw new Error('Issue level incorrect');
    // Test Ad status changes
    ad.status = 'PAUSED';
    ad.effectiveStatus = 'PAUSED';
    await ad.save();
    const pausedAd = await ad_1.AdModel.findOne({ adId: ad.adId });
    if (pausedAd?.status !== 'PAUSED')
        throw new Error('Ad status not updated');
    if (pausedAd?.effectiveStatus !== 'PAUSED')
        throw new Error('Ad effective status not updated');
    ...
    if (disapprovedAd.effectiveStatus !== 'DISAPPROVED')
        throw new Error('Disapproved status not set');
    ...
    const snapshot = await performance_snapshot_1.PerformanceSnapshotModel.create({
        entityType: 'AD',
        entityId: ad.adId,
    ...
    if (!snapshot.entityId)
        throw new Error('PerformanceSnapshot creation failed');
    if (snapshot.impressions !== 10000)
        throw new Error('Impressions incorrect');
    if (snapshot.conversions !== 25)
        throw new Error('Conversions incorrect');
    ...
    if (ctr < 4.9 || ctr > 5.1)
        throw new Error('CTR calculation incorrect'); // ~5%
    if (cpa < 10 || cpa > 10.1)
        throw new Error('CPA calculation incorrect'); // ~$10
    if (roas < 4.9 || roas > 5.1)
        throw new Error('ROAS calculation incorrect'); // ~5.0
    // Test PerformanceSnapshot unique constraint (date per entity)
    try {
        await performance_snapshot_1.PerformanceSnapshotModel.create({
            entityType: 'AD',
            entityId: ad.adId,
    ...
    const log = await optimization_log_1.OptimizationLogModel.create({
        action: 'PAUSE',
        entityType: 'AD',
        entityId: ad.adId,
    ...
    if (!log.action)
        throw new Error('OptimizationLog creation failed');
    if (log.action !== 'PAUSE')
        throw new Error('Optimization action incorrect');
    if (!log.success)
        throw new Error('Optimization success flag incorrect');
    if (!log.details || !log.details.reason)
        throw new Error('Optimization details not saved');
    // Test querying logs by entity
    const logs = await optimization_log_1.OptimizationLogModel.find({
        entityType: 'AD',
        entityId: ad.adId,
    }).sort({ executedAt: -1 });
    if (logs.length === 0)
        throw new Error('Optimization logs not found');
    if (logs[0].action !== 'PAUSE')
        throw new Error('Latest log action incorrect');
    // Test campaign with multiple ad sets and ads
    const campaign2 = await campaign_1.CampaignModel.create({
    ...
    if (activeAdSets.length !== 1)
        throw new Error('Expected 1 active ad set');
